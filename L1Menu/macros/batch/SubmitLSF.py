...
MenuFile = [
  ...
  # "menu/Menu_ETMStudy.txt",
    "menu/run_lumi_324077.csv",
]
PSColumn = [
  "menu/main-seeds_prescaled.txt",
]
 ...
 #   "ntuple/r273725_itgv59.list",
  #  "ntuple/r273728_itgv59.list",
   # "ntuple/r273730_itgv59.list",
    ...
    # "ntuple/r272818_unpack.list",
    # "ntuple/r272798_unpack.list",
     "ntuple/your_ntuple_324022.list",
]
GlobalOpt = ""
# GlobalOpt += " --doPlotEff"
...
def BSUB(Analysis, Menu_,PSColumn_, Ntuple_, Option):
    global LSFque

    # Open a pipe to the qsub command.
    #output, input = popen2('echo')
    prescale="%s%s"%(DelDir,PSColumn_)
   ...
   # cmd = "./%s %s -m %s -l %s -o %s -b %f %s" % (os.path.basename(DelExe), Option[1], Menu, Ntuple,out_name, nBunches, GlobalOpt)
    cmd = "./%s %s -u %s -m %s -l %s -o %s -b %f %s" % (os.path.basename(DelExe), Option[1], Menu, prescale , Ntuple,out_name, nBunches, GlobalOpt)
    if DryRun:
       print cmd
       return
    ...
    cd $TOP
    mkdir menu
    cp $CMSSW/src/L1TriggerDPG/L1Menu/macros/menu/run_lumi_324022.csv menu/
    cp $CMSSW/src/L1TriggerDPG/L1Menu/macros/menu/main-seeds_prescaled.txt menu/
    %s
    ls
    ...
    for menu in MenuFile:
        for ntuple in Ntuplelist:
            for opt in Options.items():
           	 for PSClmn in PSColumn:
                	#print Analysis, menu, ntuple, opt
                	BSUB(Analysis, menu, PSClmn, ntuple, opt)
